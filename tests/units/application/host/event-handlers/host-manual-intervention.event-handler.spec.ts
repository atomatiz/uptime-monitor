...
import { HostManualInterventionEventHandler } from '@application/host/event-handlers/host-manual-intervention.event-handler';
...
describe('HostManualInterventionEventHandler', () => {
    let handler: HostManualInterventionEventHandler;
    ...
    beforeEach(async () => {
        ...
        handler = moduleRef.get<HostManualInterventionEventHandler>(
            HostManualInterventionEventHandler,
        );
    ...
    it('should execute SendNotificationCommand when handling HostManualInterventionEvent', async () => {
        const event = new HostManualInterventionEvent(host.id);
    ...
    it('should throw error if host is not found in aggregate store', async () => {
        ...
        const event = new HostManualInterventionEvent('non-existent-id');
